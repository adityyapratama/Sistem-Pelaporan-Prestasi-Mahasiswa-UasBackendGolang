...
func (r *PostgresLectureRepository) Update(ctx context.Context, l *models.Lecture) error {
	query := `
		UPDATE lecturers 
		SET lecturer_id = $1, department = $2
		WHERE id = $3
	`
	result, err := r.db.ExecContext(ctx, query, l.LecturerID, l.Department, l.ID)
	if err != nil {
		return err
	}
...
func (r *PostgresLectureRepository) Delete(ctx context.Context, id uuid.UUID) error {
